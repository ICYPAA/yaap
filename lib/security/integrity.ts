...
class AppIntegrityChecker {
  ...
  private isDebugging(): boolean {
    ...
    // @ts-ignore
    if (global.location && global.location.href && global.location.href.includes('debugger')) {
      return true;
    }
  ...
  private async verifyAppSource(): Promise<boolean> {
    try {
      if (Platform.OS === 'ios') {
        ...
        const installerPackage = await Application.getInstallationSourceAsync();
        const validSources = [
  ...
  private async saveCheckTime(): Promise<void> {
    try {
      await AsyncStorage.setItem(this.INTEGRITY_CHECK_KEY, Date.now().toString());
    } catch (error) {
   ...
   */
  showSecurityWarning(message: string, severity: 'low' | 'medium' | 'high'): void {
    const titles = {
      low: 'Notice',
